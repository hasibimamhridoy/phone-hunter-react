...
import { getDataBase } from "../../utilities/Database/Database";
...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    for (const id in storedCartItem) {
      ...
      findItem.singleProductPrice =
        findItem.price - (findItem.price * findItem.discountPercentage) / 100;
  ...
  console.log(grandFinalPrice);

  return (
    <div>
      <h1 className="text-4xl text-center mt-8 ">Check out</h1>
              ...
              <button className="mt-8 border border-transparent hover:border-gray-300 dark:bg-white dark:hover:bg-gray-900 dark:text-gray-900 dark:hover:text-white dark:border-transparent bg-gray-900 hover:bg-white text-white hover:text-gray-900 flex justify-center items-center py-4 rounded w-full">
                <div>
                  <p className="text-base leading-4">
                    Pay {grandFinalPrice.toFixed(2)} Tk
                  </p>
                </div>
              </button>
            </div>
